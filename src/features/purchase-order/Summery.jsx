...
import { getPaymentsGroupedByInvoiceService } from '../../services/invoicePaymentsService';



function Summery({ open, onClose, poId, updatePOStatusAPICall }) {
    ...
    const callInitialLists = () => {
        ...
        Promise.all([
            ...
            getPaymentsGroupedByInvoiceService(poId)
        ])
            .then(([poResp, grnResp, invoiceResp]) => {
  ...
  <>
    ...
    <Stack spacing={1}>
      {invoiceItems.map((invoice) => {
        ...
        }[invoice.payment_status] || theme.palette.grey[600];
          ...
          >
            ...
            <Box textAlign="right">
              <Typography sx={{ fontSize: '0.8rem' }}>
                Amount: ₹{parseFloat(invoice.total_invoice_amount).toFixed(2)}
              </Typography>
              <Typography sx={{ fontSize: '0.8rem' }}>
                Paid: ₹{parseFloat(invoice.total_paid).toFixed(2)}
              </Typography>
              <Box
                component="span"
                sx={{
