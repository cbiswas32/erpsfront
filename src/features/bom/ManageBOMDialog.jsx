...
import { getBOMByProductIdService, saveOrUpdateBOMService } from '../../services/bomServices';
  ...
  return (
    <Dialog open={open} onClose={handleClose} fullWidth maxWidth="md">
      ...
      <DialogContent dividers>
        <Button variant="outlined" onClick={handleAdd} startIcon={<AddCircleOutlineIcon />} sx={{ mb: 2 }}>
          Add Component
        </Button>
        {bomList.length > 0 ? (
          <Paper>
                  ...
                  <TableRow key={row.tempId}>
                    <TableCell>
                      <TextField
                        ...
                        onChange={(e) => {
                          let selectedProduct = allProducts?.find(x => x.productId ===  e.target.value ) || {}
                          handleChange(row.tempId, 'componentProductId', e.target.value)
                          handleChange(row.tempId, 'unit', selectedProduct?.unit)
                          handleChange(row.tempId, 'unitPrice', selectedProduct?.unitPrice)

                          }}
                      >
                        <MenuItem value="">-- Select --</MenuItem>
                        {allProducts.map(p => (
                          <MenuItem key={p.productId} value={p.productId} unit={p.unit}>
                    ...
                    <TableCell>
                      <TextField
                        ...
                        onChange={(e) => handleChange(row.tempId, 'quantity', e.target.value)}
                          slotProps={{
                                              input: {
                                                endAdornment:   <Typography sx={{fontSize: '0.7rem', p:1, }}>{row.unit}</Typography> ,
                                              },
                                            }}
                      />
                    </TableCell>
                    ...
                    <TableCell>
                      <TextField
                        ...
                        fullWidth
                        disabled
                        value={ (parseFloat(row.unitPrice) || 0) * (parseFloat(row.quantity) || 0).toFixed(2)}
                        //onChange={(e) => handleChange(row.tempId, 'quantity', e.target.value)}
                        
                      />
                    </TableCell>
                    <TableCell>
                      <IconButton color="error" onClick={() => handleDelete(row.tempId)}>
                        <DeleteIcon />
                      </IconButton>
                    </TableCell>
                  </TableRow>
      ...
      <DialogActions>
        ...
        <Button variant="contained" onClick={handleSave}>Save BOM</Button>
      </DialogActions>
    </Dialog>
  );
}
