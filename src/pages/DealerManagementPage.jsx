...
import {
  ...
  MenuItem, Chip, Avatar, Divider 
} from '@mui/material';
import PageWrapper from '../layouts/PageWrapper';
import { useUI } from '../context/UIContext';
import { Add, Edit, Delete, Visibility, Search } from '@mui/icons-material';
import { getAllDealersService, saveOrUpdateDealerService } from '../services/dealerService';
...
export default function DealerManagementPage() {
  ...
  useEffect(() => {
    ...
    hideLoader()
         ...
         const access = getAcceessMatrix('Dealer and Customer Management', 'Dealer Management');
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!selectedDealer.dealerCode?.trim()) newErrors.dealerCode = 'Dealer Code is required';
    if (!selectedDealer.dealerName?.trim()) newErrors.dealerName = 'Dealer Name is required';
   ...
   selectedDealer?.addresses?.forEach((addr, index) => {
      ...
      if (!addr.addressType?.trim()) newErrors[`addressType_${index}`] = `${prefix}: Address Type is required`;
      if (!addr.addressLine1?.trim()) newErrors[`addressLine1_${index}`] = `${prefix}: Address Line 1 is required`;
      if (!addr.city?.trim()) newErrors[`city_${index}`] = `${prefix}: City is required`;
      if (!addr.stateName?.trim()) newErrors[`state_${index}`] = `${prefix}: State is required`;
      if (!addr.district?.trim()) newErrors[`district_${index}`] = `${prefix}: District is required`;
      if (!addr.pincode?.trim()) newErrors[`pincode_${index}`] = `${prefix}: Pincode is required`;
      if (!addr.country?.trim()) newErrors[`country_${index}`] = `${prefix}: Country is required`;
    });
      ...
      selectedDealer?.contacts?.forEach((contact, index) => {
        ...
        if (!contact.contactName?.trim()) newErrors[`contactName_${index}`] = `${prefix}: Name is required`;
        if (!contact.designation?.trim()) newErrors[`designation_${index}`] = `${prefix}: Designation is required`;
        //if (!contact.email?.trim()) newErrors[`email_${index}`] = `${prefix}: Email is required`;
        if (!contact.phone?.trim()) newErrors[`phone_${index}`] = `${prefix}: Phone is required`;
      });

        // Bank Details
  selectedDealer.bankDetails.forEach((bank, index) => {
    const prefix = `Bank ${index + 1}`;
    if (!bank.accountHolderName?.trim()) newErrors[`accountHolderName_${index}`] = `${prefix}: Account Holder Name is required`;
    if (!bank.accountNumber?.trim()) newErrors[`accountNumber_${index}`] = `${prefix}: Account Number is required`;
    if (!bank.ifscCode?.trim()) newErrors[`ifscCode_${index}`] = `${prefix}: IFSC Code is required`;
    if (!bank.bankName?.trim()) newErrors[`bankName_${index}`] = `${prefix}: Bank Name is required`;
    if (!bank.branchName?.trim()) newErrors[`branchName_${index}`] = `${prefix}: Branch Name is required`;
  });

   Object.keys(newErrors).length > 0 && showSnackbar(Object.values(newErrors), 'error')
    setErrors(newErrors);
  ...
  const handleAddSectionItem = (key, newItem) => {
    setSelectedDealer({ ...selectedDealer, [key]: [...selectedDealer[key], newItem] });
  };
          ...
          >
            <CardContent>
              <Stack direction="row" spacing={1} alignItems="center" mb={1}>
                <Avatar sx={{ bgcolor: dealer.status ? 'green' : 'red', width: 32, height: 32 }}>
                  <BusinessIcon fontSize="small" />
                </Avatar>
                <Box>
                  <Typography variant="h6">{dealer.dealerName}</Typography>
                  <Chip
                  ...
                  <Chip
                    ...
                    icon={dealer.status ? <CheckCircleIcon /> : <CancelIcon />}
                  />
                </Box>
              </Stack>
              ...
              <Stack spacing={1} mt={2}>
                <Stack direction="row" spacing={1} alignItems="center">
                  ...
                  <Typography variant="body2">{dealer.email || 'N/A'}</Typography>
                </Stack>
                <Stack direction="row" spacing={1} alignItems="center">
                  <PhoneIcon fontSize="small" />
                  <Typography variant="body2">{dealer.phone || 'N/A'}</Typography>
                </Stack>
                <Stack direction="row" spacing={1} alignItems="center">
                  <PersonIcon fontSize="small" />
                  <Typography variant="body2">{userIDUserNameMap[dealer.reportingToUserId] || 'N/A'}</Typography>
                </Stack>
              </Stack>
            </CardContent>

            <CardActions sx={{display: 'flex', alignItems: 'flex-end', flexDirection: 'column-reverse', gap: 1}}>
 ...
 <Button
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} fullWidth maxWidth="md">
        <DialogTitle>{isViewMode ? 'View Dealer' : selectedDealer.dealerId ? 'Edit Delaer' : 'Add Dealer'}</DialogTitle>
        <DialogContent>
          ...
          <Box m={2}>
            {activeTab === 0 && (
              <Grid container spacing={2} mt={1}>
                <Grid item xs={12} sm={6}>
                  <TextField
                   disabled={isViewMode}
                    ...
                    onChange={(e) => setSelectedDealer({ ...selectedDealer, dealerCode: e.target.value })}
                  />
                </Grid>
                <Grid item xs={12} sm={6}>
                  <TextField
                ...
                <Grid item xs={12} sm={6}>
                  <TextField
                   disabled={isViewMode}
                    ...
                    onChange={(e) => setSelectedDealer({ ...selectedDealer, gstin: e.target.value })}
                  />
                </Grid>
                <Grid item xs={12} sm={6}>
                  <TextField
                   disabled={isViewMode}
                    ...
                    onChange={(e) => setSelectedDealer({ ...selectedDealer, pan: e.target.value })}
                  />
                </Grid>
                <Grid item xs={12} sm={6}>
                  <TextField
                  ...
                  >
                    {userList?.map((user) => (
                      <MenuItem key={user.userId} value={user.userId}>
                        {user.userFirstname} {user.userMiddlename} {user.userLastname} ({user.loginId}) - {user.roleShortname}
                      </MenuItem>
                    ))}
                  </TextField>
                </Grid>
                ...
                <Grid item xs={12} sm={6}>
                  <TextField select  disabled={isViewMode} label="Status" value={selectedDealer.status} onChange={(e) => setSelectedDealer({ ...selectedDealer, status: e.target.value })} fullWidth  >
        ...
        <DialogActions>
          ...
          {!isViewMode && <Button variant="contained" onClick={handleSave}>Save</Button>}
        </DialogActions>
      </Dialog>
